...
namespace Descending.Player
{
    public class AttackController : MonoBehaviour
    {
        ...
        public void ProcessLeftClick(Vector3 hitPoint, Enemy target)
        {
            ...
            if (hero == null || meleeWeapon == null) return;
            
            distance = Vector3.Distance(transform.position, target.transform.position);
            
            if(meleeWeapon.Key != "" && distance <= meleeWeapon.GetWeaponData().Range)
            {
                MeleeAttack(meleeWeapon, hero, target);
                StartCoroutine(TriggerDelay(1f));
            }
            else if (rangedWeapon.Key != "" && distance <= rangedWeapon.GetWeaponData().Range)
            {
